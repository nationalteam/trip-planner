...
describe('/api/maps/config', () => {
  const originalNextPublic = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
  ...
  afterAll(() => {
    process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY = originalNextPublic;
    process.env.GOOGLE_MAPS_API_KEY = originalServerKey;
  });

  it('returns NEXT_PUBLIC_GOOGLE_MAPS_API_KEY when available', async () => {
    ...
    expect(body).toEqual({ apiKey: 'public-key' });
  });

  it('falls back to GOOGLE_MAPS_API_KEY when NEXT_PUBLIC key is missing', async () => {
    process.env.GOOGLE_MAPS_API_KEY = 'server-key';
